...
dup_rows <- admin_tab %>%
  ...
  arrange(intervention_specific_clean, status, subpopulation, required) %>%
  ungroup()

# Index of rows that are within 2 days of the previous row and that previous row
row_index <- unique(c(which(dup_rows$time_diff < 3), which(dup_rows$time_diff < 3) - 1))
